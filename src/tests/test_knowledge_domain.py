...
def test_knowledge_file_auto_generates_id():
    kf = KnowledgeFile(name="x", description="", content="", file_type=_FILE_TYPE_MD, scope=SCOPE_PROJECT)
    assert isinstance(kf.id, str)
    assert len(kf.id) == 36  # uuid4


def test_knowledge_file_two_instances_have_different_ids():
    a = KnowledgeFile(name="a", description="", content="", file_type=_FILE_TYPE_MD, scope=SCOPE_PROJECT)
    b = KnowledgeFile(name="b", description="", content="", file_type=_FILE_TYPE_MD, scope=SCOPE_PROJECT)
    assert a.id != b.id
...
def test_knowledge_file_created_at_is_datetime():
    kf = KnowledgeFile(name="x", description="", content="", file_type=_FILE_TYPE_TXT, scope=SCOPE_PROJECT)
